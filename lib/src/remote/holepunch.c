...
// The chain is recovered from the same host that hands out the node list: it
// serves the identical wildcard leaf with a complete chain, so it must stay in
// step with the API URL below rather than being a second literal.
#define WS_BOOTSTRAP_HOST "mobile-pushcl.np.communication.playstation.net"
static const char ws_chain_source_fqdn[] = WS_BOOTSTRAP_HOST;
static const char ws_fqdn_api_url[] = "https://" WS_BOOTSTRAP_HOST "/np/serveraddr?version=2.1&fields=keepAliveStatus&keepAliveStatusType=3";
...
static void* websocket_thread_func(void *user) {
    ...
    /* A repaired chain can only be applied if the TLS backend accepts a CA
     * blob. Probing with NULL is free: the support check runs before the blob
     * is stored, so an unsupported backend answers without side effects. */
    bool aia_blob_supported =
        curl_easy_setopt(curl, CURLOPT_CAINFO_BLOB, NULL) == CURLE_OK;
    if(!aia_blob_supported)
        CHIAKI_LOGW(session->log, "websocket_thread_func: TLS backend does not accept a CA blob, chain repair unavailable");

ws_connect_attempt:
    {
        size_t aia_blob_len = 0;
        char *aia_blob = aia_blob_supported ? chiaki_aia_blob_take(&aia_blob_len) : NULL;
        if(aia_blob)
        {
            ...
            CURLcode blob_res = curl_easy_setopt(curl, CURLOPT_CAINFO_BLOB, &ca_blob);
    ...
    {
        aia_attempted = true;
        CHIAKI_LOGE(session->log, "websocket_thread_func: %s failed and certificate chain repair was SKIPPED because the TLS backend will not accept a CA blob; a leaf-only chain cannot be repaired on this build",
            session->ws_fqdn);
    }
