...
CHIAKI_EXPORT ChiakiErrorCode chiaki_stream_connection_run(ChiakiStreamConnection *stream_connection, chiaki_socket_t *socket)
{
	...
	takion_info.psn_wrapper_type = chiaki_service_type_is_cloud(session->service_type)
		...
		: 0;
	takion_info.is_ping_handshake = false;
...
{
	ChiakiSession *session = stream_connection->session;
	const char *launch_spec_b64_ptr = NULL;
	...
	{
		if(!session->cloud_launch_spec || !*session->cloud_launch_spec)
	...
	ChiakiErrorCode err = chiaki_ecdh_get_local_pub_key(&session->ecdh,
